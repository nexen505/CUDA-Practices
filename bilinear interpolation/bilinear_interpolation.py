...
if __name__ == "__main__":
    im = imageio.imread("lozman_16_0.png")
    ...
    # resized by scikit image
    plt.imshow(skimage.transform.resize(im, tuple(enlargedShape)))
    plt.show()

    # image interpolated manually
    for r in range(enlargedImg.shape[0]):
        for c in range(enlargedImg.shape[1]):
            orir = r * rowScale  # Find position in original image
            oric = c * colScale
            enlargedImg[r, c] = get_bilinear_pixel(im, oric, orir)
