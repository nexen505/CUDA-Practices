...
def interpolate_image_by_cuda(image: np.ndarray, scale: int = 2):
    ...
    enlarged_image_shape = get_image_enlarged_shape(image, scale)
    result = gpuarray.to_gpu(np.zeros((enlarged_image_shape[0], enlarged_image_shape[1]), dtype=np.uint32))
...
def get_images_files(folder: str = './data'):
    images_files = []
    for ignoredRoot, ignoredDirs, files in os.walk(folder):
        images_files += [f'{folder}/{file}' for file in files if file.endswith(".png")]
    return images_files
